...
def farmer(request):
    ...
    if curuser and role == 'far':
        ...
        # veg = request.POST.get('status')
        farmid = curuser['localId']
        if additem:
            lettersD = string.digits
            oid = (''.join(random.choice(lettersD) for i in range(3)))
            catname = request.POST.get('Item Name')
            vprice = request.POST.get('price')
            vname = request.POST.get('status')
        ...
        if ucid:
            ...
            for i in upcomeinfo:
                det = database.child('Upcoming task').child(farmid).child(i).get().val()
                diction = dict(det)
                # print('ssssssssssssssssssssssssssssssssssssssssssssss:' ,diction)
                upcometaskdetails = diction
        # print('upcometaaaaaaaaaaaaaaaaaaaaaaaaaask det:', upcometaskdetails)
        details2 = {
            'det': details,
            'anidet': anidetails,
            'logdet': logdetails,
...
def crop(request):
    return render(request, 'posthar.html')
